...
export const authConfig = {
    callbacks: {
        jwt: async ({token}) => {
          if(token && token.sub) {
            const existingUser = await db.user.findFirst({where: { OR: [{oauthId: token.sub}, { id: !isNaN(Number(token.sub)) ? parseInt(token.sub) : undefined}, { email: token.email!}]}, select: {id: true}})
            if(existingUser) {
